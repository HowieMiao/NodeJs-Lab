...
app.get('/player_info', function(req, res) {
	var player_year = req.query.player_year;
...
app.get('/players', function(req, res) {
	var player_year = req.query.player_year;
	console.log(player_year)
	var q1
	if(player_year != null){
		q1 = `SELECT name, id FROM football_players WHERE year = '${player_year}';`
	} else {
		q1 = `SELECT name, id FROM football_players;`
	}
...
app.get('/players/:player_choice', function(req, res) {
	...
	var q3 = `SELECT COUNT(*) FROM football_games WHERE '${req.query.player_choice}' =ANY(players);`
	...
	.then(info =>  {
		...
		res.render('pages/player_info',{
				my_title: "Players",
				players:info[0], // Return all players
				player: info[1],// Return specific player
				games: info[2]
			})
	})
	.catch(err => {
...
app.post('/players/add', function(req, res){
	...
	var pass = req.body.player_passing_yards
	var rush = req.body.player_rushing_yards
	var receive = req.body.player_receiving_yards

	var q = `INSERT INTO football_players(name, year, major, passing_yards, rushing_yards, receiving_yards) VALUES('${name}', '${year}', '${major}', '${pass}', '${rush}', '${receive}');`
	var all_players = 'SELECT id, name FROM football_players;'
	var player = `SELECT * FROM football_players WHERE name = '${ name}';`
...
app.post('/games/add', function(req, res){
	...
	var q = `INSERT INTO football_game(visitor_name, home_score, visitor_score, game_date, players) VALUES('${visitor_name}', '${home_score}', '${visitor_score}', '${game_date}', '${players}');`
	var all_players = 'SELECT id, name FROM football_players;'
